...
decisions this platform made explicitly, versus the old platform's OpenShift SCC-based
model where the shared build identity had `allowPrivilegedContainer: true`.
...
worth evaluating next - it was the old platform's choice, and rootless mode support has
matured, but it's less battle-tested specifically for the "no shell privileges at all"
constraint kaniko was built around. Either way, this is the one Task in the whole
