...
to an app's own `./integration-test.sh` (`run-integration-tests.yaml`'s own header
...
Application secrets reach a TestWorkflow a different way: `run-integration-tests.yaml`
...
The workflow's own YAML (`platform/<suite>.yaml` in the app repo) references it via a
...
- **TestWorkflow/TestWorkflowExecution** (`platform-cicd-control-plane`'s
  ...
  Application's own chart): deliberately NOT resourceNames-restricted. Suite names are
  ...
  reintroducing an operator step for every new suite, defeating the point. Accepted,
  ...
  force-rewritten to `<trusted-app-name>-<suite>` before applying (app-name comes from
...
- `platform/<suite>.yaml` becomes a new, self-service, developer-owned convention in
